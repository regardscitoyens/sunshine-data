...
class ConventionItem(scrapy.Item):
    type = scrapy.Field() 
    range = scrapy.Field() 
    labo = scrapy.Field()
...
class TSConventionsSpider(TSSpider):
    name = "ts_conventions"
    ...
    def entrepriseAvantagesTable(self, response):
        ...
        yield Request(
            url='https://www.transparence.sante.gouv.fr/flow/rechercheEntreprises?execution='+execvar+'&j_idt17=j_idt17&j_idt17%3Aj_idt23=Afficher+les+Conventions&javax.faces.ViewState='+execvar,
            callback=self.entrepriseConventionsTable, dont_filter=True
            )
        return;

    def entrepriseConventionsTable(self, response):
        ...
        if response.css('table'):
            ...
            if (len(response.css('input.btn-next::attr(disabled)').extract()) == 0 and len(response.css('.btn-page input').extract()) > 1):
                nexturl = 'https://www.transparence.sante.gouv.fr/flow/rechercheEntreprises?execution='+tableexecvar+'&j_idt17=j_idt17&j_idt17%3Aj_idt161=Suivant&javax.faces.ViewState='+tableexecvar
                yield Request(url=nexturl, callback=self.entrepriseConventionsTable, dont_filter=True)
            else:
