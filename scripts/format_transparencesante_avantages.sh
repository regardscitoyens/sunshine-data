...
cat data/raw/transparencesante_avantages.csv | sed 's/\r//' | sed 1d | iconv -f utf8 -t utf8//IGNORE | perl -e 'while(<STDIN>){chomp; while(s/"([^"]*),([^"]*)"/"\1 -\2"/g){}; @a = split /,/; @s = split /\//, $a[3] ; $a[3] = $s[2]."-".$s[1]."-".$s[0]; print join ",", @a; print "\n"; }' | awk -F ',' '{print $2","$1","$3","$7","$2",,,,,"$6","$4","$5","$7}' | sed 's/  */ /g' | sed 's/, /,/g' | sed 's/ ,/,/g' >> data/formatted/transparencesante_avantages.formatted.csv


#sed "s/Forfait *journée d[^é]*étude/Forfait journée d'étude/" | sed "s/FRAIS D'HOSPITALIT[^,]*,/FRAIS D'HOSPITALITE,/" | sed "s/’/'/g" | sed 's/ÃƒÂ©/e/g' | sed 's/MAZET Anne-Aur[^,]lie/MAZET Anne-Aurelie/'
