...
cat data/raw/transparencesante_avantages.csv | sed 's/\r//' | sed 1d | perl -e 'while(<STDIN>){@a = split /,/; @s = split /\//, $a[3] ; $a[3] = $s[2]."-".$s[1]."-".$s[0]; print join ",", @a; }' | awk -F ',' '{print $2","$1","$3","$7","$2",,,,,"$6","$4","$5","$7}' | sed 's/  */ /g' | sed 's/, /,/g' | sed 's/ ,/,/g' >> data/formatted/transparencesante_avantages.formatted.csv
