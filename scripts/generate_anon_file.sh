...
awk -F ',' '{print $1","$2","$3","$7","$16","$17","$13","$14","$12","$9","$10","$18}' data/all.unames.csv > data/tmp/all.anonymes.csv
head -n 1 data/tmp/all.anonymes.csv > data/all.anonymes.csv
sed 1d data/tmp/all.anonymes.csv | sort -u -t ',' -k 1,11 >> data/all.anonymes.csv
